...
tabella_confronti <- function(dataset, classe, R = NULL, seed = NULL,...) {
  ...
  # dataframe
  data <- data.frame(
    MSE1_5nn = c(boot_5nn$MSE , b632_5nn$MSE, b632_plus_5nn$MSE, cv_5nn$MSE, loocv_5nn$MSE, bcv_5nn$MSE, mccv_5nn$MSE), 
    Bias1_5nn = c(boot_5nn$Bias , b632_5nn$Bias, b632_plus_5nn$Bias, cv_5nn$Bias, loocv_5nn$Bias, bcv_5nn$Bias, mccv_5nn$Bias),
    MSE2_7nn = c(boot_7nn$MSE , b632_7nn$MSE, b632_plus_7nn$MSE, cv_7nn$MSE, loocv_7nn$MSE, bcv_7nn$MSE, mccv_7nn$MSE), 
    Bias2_7nn = c(boot_7nn$Bias , b632_7nn$Bias, b632_plus_7nn$Bias, cv_7nn$Bias, loocv_7nn$Bias, bcv_7nn$Bias, mccv_7nn$Bias),
    MSE3_tree = c(boot_tree$MSE , b632_tree$MSE, b632_plus_tree$MSE, cv_tree$MSE, loocv_tree$MSE, bcv_tree$MSE, mccv_tree$MSE), 
    Bias3_tree = c(boot_tree$Bias , b632_tree$Bias, b632_plus_tree$Bias, cv_tree$Bias, loocv_tree$Bias, bcv_tree$Bias, mccv_tree$Bias),
    MSE4_lda = c(boot_lda$MSE , b632_lda$MSE, b632_plus_lda$MSE, cv_lda$MSE, loocv_lda$MSE, bcv_lda$MSE, mccv_lda$MSE), 
    Bias4_lda = c(boot_lda$Bias , b632_lda$Bias, b632_plus_lda$Bias, cv_lda$Bias, loocv_lda$Bias, bcv_lda$Bias, mccv_lda$Bias),
    MSE3_qda = c(boot_qda$MSE , b632_qda$MSE, b632_plus_qda$MSE, cv_qda$MSE, loocv_qda$MSE, bcv_qda$MSE, mccv_qda$MSE), 
    Bias3_qda = c(boot_qda$Bias , b632_qda$Bias, b632_plus_qda$Bias, cv_qda$Bias, loocv_qda$Bias, bcv_qda$Bias, mccv_qda$Bias)
  )
  
  data <- data %>%
    mutate(across(starts_with("MSE"), sqrt))
  rownames(data) <- c("boot", ".632", ".632+", "10-fold CV", "LOOCV", "BCV", "MC-CV")
